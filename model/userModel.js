...
const {
  registerValidation,
  loginValidation,
} = require("../authentication/validations");
...
exports.deleteAccount = async (data) => {
  ...
  return (async () => {
    await pool.query(
      `UPDATE public.users SET active='false' WHERE loginid = ${userid.loginid}`
    );
...
exports.changePassword = async (data) => {
  ...
  userid = jwt.verify(data.headers["auth-token"], process.env.TOKEN_SECRET);
  return (async () => {
    const { rows: password } = await pool.query(
      `SELECT password FROM public.users WHERE loginid = ${userid.loginid}`
    );
    ...
    if (validPassword) {
      await pool.query(
        `UPDATE public.users SET password='${hashedNewPassword}' WHERE loginid = ${userid.loginid}`
      );
      resdata = {
        status: 200,
...
exports.getEmail = async (data) => {
  ...
  return (async () => {
    const { rows: email } = await pool.query(
      `SELECT email FROM public.users WHERE loginid = ${userid.loginid}`
    );
...
const passwordForEmail = async (request, response) => {
  return pool
    .query(
      `SELECT loginid, password FROM public.users WHERE email = '${request}'`
    )
    .then((response) => {
...
const emailExist = async (request, response) => {
  return pool
    .query(
      `SELECT EXISTS(SELECT email FROM public.users where email = '${request}' and active = true)`
    )
    .then((response) => {
...
const addingNewUser = (request) => {
  ...
  pool
    .query("SELECT max(loginid) FROM public.users")
    .then((response) => {
      var lastUserID = Number(Object.values(response.rows[0])) + 1;
      pool.query(
        `INSERT INTO public.users(loginid, email, password, active)VALUES('${lastUserID}','${email}','${password}', '${active}')`,
        (error, response) => {
          if (error) {
